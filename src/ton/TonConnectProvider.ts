...
import { ITonConnectProviderEvents } from "./walletConnectTypes";
import {
  Metadata,
  Namespace,
  UniversalProvider,
...
const OPTIONAL_EVENTS = ["message", "disconnect", "connect"];
...
export function buildNamespaces(params: NamespacesParams): {
  ...
  const eventsRequiringPermissions = events?.filter(
    (event) => !REQUIRED_EVENTS.includes(event)
  );
  const methodsRequiringPermissions = methods?.filter(
    (event) => !REQUIRED_METHODS.includes(event)
  );
...
export class EthereumProvider implements IEthereumProvider {
  ...
  public on: ITonConnectProviderEvents["on"] = (event, listener) => {
    this.events.on(event, listener);
    return this;
  };

  public once: ITonConnectProviderEvents["once"] = (event, listener) => {
    this.events.once(event, listener);
    return this;
  };

  public removeListener: ITonConnectProviderEvents["removeListener"] = (
    event,
    listener
  ) => {
  ...
  public off: ITonConnectProviderEvents["off"] = (event, listener) => {
  ...
  protected loadConnectOpts(opts?: ConnectOps) {
    ...
    if (optionalChains && isValidArray(optionalChains)) {
      ...
      this.rpc.optionalChains = optionalChains?.map((chain) =>
        this.formatChainId(chain)
      );
